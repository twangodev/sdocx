...
Table flexible bits 0 and 1 are `f32` vertical and horizontal cell padding.
Bit 2 stores a `u32` column count followed by `f32` widths. Bit 3 stores a `u32`
row count followed by `u32` payload size plus row bytes for each row. The native
writer also emits fields 4 through 13; full semantics remain to be mapped.
Existing `RichTextTable` decoding keeps columns, rows and cells and skips those
additional styles. The original embedded object bytes remain available through
`RichTextObjectSpan.object_data`.
...
height is `f32::MAX`, and default minimum height is zero. These properties are
not yet exposed by `RichTextTableRow`.
...
`TableBorder::NewGetBinary` (`0x3c31a0`–`0x3c31bc`). The border structure needs
separate decoding before it can inform rendering. Its bytes cannot be treated
as additional cell text.
...
beyond the record, wider field masks and nested cells. The existing embedded
table/code-block tests continue to pass. These tests establish bounds and
structure, not Samsung rendering fidelity.

Next APK work can map table style fields 4–13 and border records, expose row
height constraints, and add standalone object decoding with explicit diagnostics
for unsupported features. The renderer currently uses approximations for
embedded table/code-block styling; raw inherited shape data and additional
styles must be considered before claiming equivalent standalone rendering.
