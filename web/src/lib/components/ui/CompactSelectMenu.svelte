...
<DropdownMenu {items} {onAction} {align} size="compact">
	{#snippet children({ props })}
		<button
			...
			aria-label={label}
			{disabled}
			class="flex h-6 cursor-pointer items-center gap-1 rounded px-1.5 font-mono text-[11px] tabular-nums text-text outline-none transition-colors hover:bg-surface disabled:cursor-default disabled:opacity-40 {className}"
		>
