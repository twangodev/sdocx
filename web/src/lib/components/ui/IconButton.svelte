<script lang="ts">
	...
	const buttonClass = $derived(
		`flex ${sizes[size]} cursor-pointer items-center justify-center rounded transition-colors hover:bg-surface disabled:cursor-default disabled:opacity-40 ${tones[tone]} ${active ? 'text-accent' : 'text-muted'} ${className}`
	);
</script>
