<script lang="ts">
	...
	const buttonClass = $derived(
		`flex ${sizes[size]} shrink-0 cursor-pointer items-center justify-center rounded p-0 leading-none transition-colors hover:bg-surface disabled:cursor-default disabled:opacity-40 ${tones[tone]} ${active ? 'bg-surface text-text' : 'text-muted'} ${className}`
	);
</script>
