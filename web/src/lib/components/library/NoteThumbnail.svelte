<script lang="ts">
	...
	$effect(() => {
		...
		let objectUrl = '';
		url = '';
		if (thumbnail) void assets.read(thumbnail).then((file) => {
			if (!active) return;
...
<style>
	.thumbnail { display: grid; place-items: center; height: 100%; min-height: 0; background: var(--site-surface); color: var(--site-muted); overflow: hidden; }
