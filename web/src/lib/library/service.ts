...
export class LibraryService {
	constructor(
		...
		private readonly exclusive: ExclusiveAccess = async (action) => await navigator.locks.request('sdocx-library', action),
		private readonly changed: () => void = () => {}
	) {}

	private async mutate<T>(action: () => Promise<T>): Promise<T> {
		const result = await this.exclusive(action);
		this.changed();
		return result;
	}

	async importDocument(prepared: PreparedDocument, collectionId?: string): Promise<{ document: LibraryDocument; duplicate: boolean }> {
		return this.mutate(async () => {
			const existing = await this.catalog.documents.where('contentHash').equals(prepared.contentHash).first();
			if (existing) {
			...
			await this.catalog.transaction('rw', [this.catalog.documents, this.catalog.memberships, this.catalog.collections], async () => {
	...
	async deleteCollection(id: string): Promise<void> {
		await this.mutate(() => this.catalog.transaction('rw', [this.catalog.collections, this.catalog.memberships], async () => {
	...
	private async addMemberships(ids: string[], collectionId?: string): Promise<void> {
		...
		if (!await this.catalog.collections.get(collectionId)) throw new Error('This collection no longer exists.');
		const documents = await this.catalog.documents.bulkGet(ids);
		await this.catalog.memberships.bulkPut(documents.flatMap((document) => document ? [{ documentId: document.id, collectionId }] : []));
	}

	async setMembership(ids: string[], collectionId: string, included: boolean): Promise<void> {
		await this.mutate(() => this.catalog.transaction('rw', [this.catalog.memberships, this.catalog.documents, this.catalog.collections], async () => {
	...
	async setFavorite(ids: string[], favorite: boolean): Promise<void> {
		await this.mutate(() => this.catalog.transaction('rw', this.catalog.documents, async () => {
	...
	async deleteDocuments(ids: string[]): Promise<void> {
		await this.mutate(async () => {
			await this.catalog.transaction('rw', [this.catalog.documents, this.catalog.memberships, this.catalog.pendingDeletes], async () => {
				for (const id of ids) {
					const document = await this.catalog.documents.get(id);
					if (!document) continue;
					await this.catalog.pendingDeletes.bulkPut([document.original, document.thumbnail].filter((name): name is string => !!name).map((name) => ({ name })));
	...
	async recover(): Promise<void> {
		await this.exclusive(async () => {
			...
			const referenced = new Set(documents.flatMap((document) => [document.original, document.thumbnail]));
